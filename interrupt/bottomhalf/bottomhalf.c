...
/* GPIO4に接続されたLED(初期状態はLOW=消灯) */
static struct gpio leds[] = { { 4, GPIOF_OUT_INIT_LOW, "LED 1" } };

/* GPIO17,18に接続されたボタン(入力用) */
...
/**
 * @brief Workqueueが実行されたときに呼ばれる関数
 */
...
/* bottomhalf_workというworkqueueを登録 */
...
static irqreturn_t button_isr(int irq, void *data) {
	...
	/* ボトムハーフとして, bottomhalf_work_fn()を非同期で実行 */
...
static int __init bottomhalf_init(void) {
	...
	/* ボタン2の状態を取得(0=押されていない, 1=押されている) */
	pr_info("Current button2 value: %d\n", gpio_get_value(buttons[1].gpio));
